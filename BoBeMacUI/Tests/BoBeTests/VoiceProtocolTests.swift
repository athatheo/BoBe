...
struct TtsFrameHeaderTests {
    ...
    @Test func parsesChunkIdBigEndian() {
        let frame = encode(chunkId: 0x0102_0304_0506_0708, flags: 0, payload: [0xAA, 0xBB])
        let parsed = TtsFrameHeader.parse(frame)
        #expect(parsed != nil)
        #expect(parsed?.header.chunkId == 0x0102_0304_0506_0708)
    }

    @Test func parsesEmptyPayload() {
        let frame = encode(chunkId: 42, flags: 0, payload: [])
        let parsed = TtsFrameHeader.parse(frame)
        #expect(parsed?.payload.count == 0)
        #expect(parsed?.header.chunkId == 42)
    }

    @Test func parsesPayloadPreservingBytes() {
        let payload: [UInt8] = [0x01, 0x02, 0x03, 0xFF, 0xFE]
        let frame = encode(chunkId: 1, flags: 0, payload: payload)
        let parsed = TtsFrameHeader.parse(frame)
    ...
    @Test func fillerFlagDetected() {
        let frame = encode(chunkId: 1, flags: TtsFrameHeader.flagFiller, payload: [])
        let parsed = TtsFrameHeader.parse(frame)
        #expect(parsed?.header.isFiller == true)
        #expect(parsed?.header.isFirstOfTurn == false)
    }

    @Test func firstOfTurnFlagDetected() {
        let frame = encode(chunkId: 1, flags: TtsFrameHeader.flagFirstOfTurn, payload: [])
        let parsed = TtsFrameHeader.parse(frame)
        #expect(parsed?.header.isFirstOfTurn == true)
        #expect(parsed?.header.isFiller == false)
    }

    @Test func bothFlagsCoexist() {
        let flags = TtsFrameHeader.flagFiller | TtsFrameHeader.flagFirstOfTurn
        let frame = encode(chunkId: 1, flags: flags, payload: [])
        let parsed = TtsFrameHeader.parse(frame)
        #expect(parsed?.header.isFiller == true)
        #expect(parsed?.header.isFirstOfTurn == true)
    }
...
struct ServerVoiceMessageTests {
    ...
    @Test func decodesStateMessage() throws {
    ...
    @Test func decodesTruncate() throws {
    ...
    @Test func unknownTypeBecomesUnknownVariant() throws {
...
struct ClientVoiceMessageEncodingTests {
    ...
    @Test func helloEncodesAllOptionalsWhenSet() throws {
    ...
    @Test func helloOmitsNilOptionals() throws {
        let obj = try encode(.hello(
            sessionId: "s1",
            playbackRate: 24_000,
            voiceId: nil,
