...
pub trait GoalRepository: Send + Sync {
    ...
    async fn find_by_status(&self, status: GoalStatus, enabled_only: bool) -> Result<Vec<Goal>, AppError>;
    async fn find_active(&self, enabled_only: bool) -> Result<Vec<Goal>, AppError>;
    async fn find_enabled(&self) -> Result<Vec<Goal>, AppError>;
    async fn find_similar(&self, embedding: &[f32], limit: i64, enabled_only: bool) -> Result<Vec<(Goal, f64)>, AppError>;
    async fn update_status(&self, id: Uuid, status: Option<GoalStatus>, enabled: Option<bool>) -> Result<Option<Goal>, AppError>;
