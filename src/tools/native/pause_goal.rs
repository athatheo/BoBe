...
pub(crate) struct PauseGoalTool {
...
#[async_trait]
impl NativeTool for PauseGoalTool {
    fn name(&self) -> &str {
        "pause_goal"
    }

    fn description(&self) -> &str {
        "Pause a goal so it won't be actively worked on"
    }
    ...
    fn parameters(&self) -> Value {
        json!({
            ...
            "properties": {
                "goal_id": {
                    "type": "string",
                    "description": "UUID of the goal to pause"
                }
            },
            "required": ["goal_id"]
        })
    ...
    async fn execute(
        ...
        if goal.status != GoalStatus::Active {
            return Err(AppError::Validation(format!(
                "Goal {goal_id} is not active (current status: {})",
                goal.status
            )));
        }
