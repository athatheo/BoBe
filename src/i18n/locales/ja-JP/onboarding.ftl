...
setup-step-model-pulling = { $model } を取得中
setup-step-model-ready = { $model } の準備完了
setup-step-vision-model-pull-failed-non-fatal = ビジョンモデルの取得に失敗しました（致命的ではありません）: { $error }
setup-step-embedding-loading = 埋め込みモデルをメモリに読み込み中...
setup-step-embedding-loaded = 埋め込みモデルを読み込みました
setup-step-embedding-warmup-failed-non-fatal = ウォームアップに失敗しました（致命的ではありません）: { $error }
...
setup-openai-error-invalid-api-key-format = OpenAI APIキーの形式が不正です。空白や改行を削除して、もう一度お試しください。
setup-openai-error-cannot-reach = OpenAIに接続できません: { $error }
setup-openai-embedding-testing = 埋め込みエンドポイントをテスト中...
setup-openai-embedding-working = 埋め込みエンドポイントは正常です
setup-openai-embedding-failed = 埋め込みテストに失敗しました: { $error }
...
setup-azure-error-invalid-value-format = Azure設定値の形式が不正です。空白や改行を削除して、もう一度お試しください。
setup-azure-error-cannot-reach = Azureエンドポイントに接続できません: { $error }
setup-azure-embedding-testing = 埋め込みをテスト中...
setup-azure-embedding-working = 埋め込みは正常です
setup-azure-embedding-failed = 埋め込みに失敗しました: { $error }
