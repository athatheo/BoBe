...
setup-error-create-data-directory = Datenverzeichnis kann nicht erstellt werden: { $error }
...
setup-step-model-pulling = { $model } wird geladen
setup-step-model-ready = { $model } bereit
setup-step-vision-model-pull-failed-non-fatal = Abruf des Vision-Modells fehlgeschlagen (nicht kritisch): { $error }
...
setup-step-embedding-warmup-failed-non-fatal = Warm-up fehlgeschlagen (nicht kritisch): { $error }
...
setup-openai-error-invalid-api-key-format = Ungültiges OpenAI-API-Schlüssel-Format. Entferne Leerzeichen/Zeilenumbrüche und versuche es erneut.
...
setup-azure-error-invalid-value-format = Ungültiges Azure-Einrichtungswertformat. Entferne Leerzeichen/Zeilenumbrüche und versuche es erneut.
